...
public class HomeController {
   ...
   /* @RequestMapping("add")
    ...
    public String getAlien(@RequestParam int aid, Model m){
        m.addAttribute("result", repo.findById(aid).orElse(null));
        return "showAliens";
    }
    ...
    public String addAlien(@ModelAttribute Alien alien, Model m) {
        System.out.println("Received: " + alien.getAid() + ", " + alien.getAname()); // Debugging
        repo.save(alien);
        m.addAttribute("alien", alien);
        return "result"; // will go to result.jsp
    }
